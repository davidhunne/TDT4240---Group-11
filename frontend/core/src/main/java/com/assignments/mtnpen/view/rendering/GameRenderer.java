...
public class GameRenderer {
    ...
    private void drawPenguin(float px, float py, boolean connected, boolean isCurrent) {
        float alpha = connected ? 1f : 0.4f;

        // Shadow
        shapeRenderer.setColor(0, 0, 0, 0.15f * alpha);
        shapeRenderer.ellipse(px - PENGUIN_BODY * 0.6f, py - PENGUIN_BODY - 2,
                PENGUIN_BODY * 1.2f, PENGUIN_BODY * 0.4f);

        // Body (black oval)
        shapeRenderer.setColor(0.1f, 0.1f, 0.12f, alpha);
        shapeRenderer.ellipse(px - PENGUIN_BODY * 0.5f, py - PENGUIN_BODY * 0.6f,
                PENGUIN_BODY, PENGUIN_BODY * 1.3f);

        // Belly (white oval)
        shapeRenderer.setColor(0.95f, 0.95f, 0.97f, alpha);
        shapeRenderer.ellipse(px - PENGUIN_BODY * 0.3f, py - PENGUIN_BODY * 0.4f,
                PENGUIN_BODY * 0.6f, PENGUIN_BODY * 0.9f);

        // Head (black circle)
        float headY = py + PENGUIN_BODY * 0.5f;
        shapeRenderer.setColor(0.1f, 0.1f, 0.12f, alpha);
        shapeRenderer.circle(px, headY, PENGUIN_BODY * 0.38f);

        // Eyes (white dots)
        shapeRenderer.setColor(1f, 1f, 1f, alpha);
        shapeRenderer.circle(px - 1.5f, headY + 1f, 1.0f);
        shapeRenderer.circle(px + 1.5f, headY + 1f, 1.0f);

        // Pupils
        shapeRenderer.setColor(0, 0, 0, alpha);
        shapeRenderer.circle(px - 1.2f, headY + 1.2f, 0.5f);
        shapeRenderer.circle(px + 1.8f, headY + 1.2f, 0.5f);

        // Beak (orange triangle)
        shapeRenderer.setColor(1f, 0.6f, 0.1f, alpha);
        shapeRenderer.triangle(px - 1.5f, headY - 1f, px + 1.5f, headY - 1f,
                px, headY - 3f);

        // Feet
        shapeRenderer.setColor(1f, 0.6f, 0.1f, alpha);
        shapeRenderer.ellipse(px - PENGUIN_BODY * 0.45f, py - PENGUIN_BODY * 0.7f,
                PENGUIN_BODY * 0.3f, PENGUIN_BODY * 0.18f);
        shapeRenderer.ellipse(px + PENGUIN_BODY * 0.15f, py - PENGUIN_BODY * 0.7f,
                PENGUIN_BODY * 0.3f, PENGUIN_BODY * 0.18f);

        // Player highlight ring
        if (isCurrent) {
            float pulse = 0.7f + 0.3f * (float) Math.sin(animationTime * 4f);
            shapeRenderer.setColor(1f, 0.2f, 0.2f, pulse * 0.4f * alpha);
            shapeRenderer.circle(px, py, PENGUIN_BODY * 1.1f);
        }
    }
